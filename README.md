# Kunta Bay Resort — Sublex Chat demo site
...
Live at **demo1.sublexchat.com**.

## What is here

Six pages of static HTML. Nothing to build and nothing to install.

| File | Page |
| --- | --- |
| `index.html` | Home |
| `rooms.html` | Rooms and rates |
| `dining.html` | Dining |
| `resort.html` | The resort |
| `getting-here.html` | Getting here |
...
1. Repository:
   `https://github.com/nfamaraceesay2018-svg/sublex-demo1-hotel.git`
...
3. Install path: leave it empty, so the files land in the document root of
   **this subdomain**. Check you are on the `demo1.sublexchat.com` entry and
...
or use the SSH URL `git@github.com:nfamaraceesay2018-svg/sublex-demo1-hotel.git` with the
...
`demo-sites/site1-hotel.mjs` in the Sublex Chat launch folder, with the shared
layout in `shared.mjs`. Run `node build.mjs`, check with `node audit.mjs`, then
